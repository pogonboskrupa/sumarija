        ...
        // ========================================
        // PRIMAČI PREGLED - Tab s dropdown selekcijom primača
        // ========================================

        async function loadPrimaciPregled() {
            document.getElementById('loading-screen').classList.add('hidden');
            document.getElementById('primaci-pregled-content').classList.remove('hidden');

            const select = document.getElementById('primaci-pregled-select');
            if (select.options.length <= 1) {
                // Popuni dropdown iz primaci endpoint-a (reuse existing cache)
                try {
                    const year = new Date().getFullYear();
                    const data = await fetchWithCache(buildApiUrl('primaci', { year }), 'cache_primaci_' + year, false, 180000);
                    if (data.primaci) {
                        data.primaci.forEach(p => {
                            const opt = document.createElement('option');
                            opt.value = p.primac;
                            opt.textContent = `${p.primac} (${p.ukupno.toFixed(2)} m³)`;
                            select.appendChild(opt);
                        });
                    }
                } catch (err) {
                    console.error('Error loading primaci list for dropdown:', err);
                }
            }
            markTabRendered('primaci-pregled');
        }

        async function loadPrimacYearlySjeca(primacName) {
            if (!primacName) return;
            const year = new Date().getFullYear();
            const cacheKey = `cache_primac_yearly_sjeca_${year}_${primacName.replace(/\s+/g, '_')}`;
            const url = buildApiUrl('primac-yearly-sjeca', { year, primac: primacName });

            const container = document.getElementById('primaci-pregled-container');
            container.innerHTML = '<div style="text-align:center;padding:40px;color:#6b7280;">⏳ Učitavam podatke...</div>';

            try {
                const data = await fetchWithCache(url, cacheKey, false, 180000);
                if (data.error) throw new Error(data.error);
                renderPrimacYearlySjeca(data, primacName, year);
            } catch (err) {
                console.error('Error in loadPrimacYearlySjeca:', err);
                container.innerHTML = `<p style="color:#dc2626;text-align:center;padding:40px;">Greška: ${err.message}</p>`;
            }
        }

        function renderPrimacYearlySjeca(data, primacName, year) {
            const container = document.getElementById('primaci-pregled-container');

            if (!data.data || data.data.length === 0) {
                container.innerHTML = `<p style="text-align:center;padding:40px;color:#6b7280;">Nema podataka za ${primacName} u ${year}. godini.</p>`;
                return;
            }

            const sortNazivi = data.sortimentiNazivi;

            // Helper: dan u sedmici
            function getDayName(dateStr) {
                const parts = dateStr.split('.');
                const day = parseInt(parts[0]);
                const month = parseInt(parts[1]) - 1;
            ...
            // Header
            let html = `<p style="color:#6b7280;font-size:13px;margin-bottom:12px;">👷 <strong>${primacName}</strong> — sječa u <strong>${year}</strong>. godini (${data.data.length} unosa)</p>`;

            // Table header
            html += '<div style="overflow-x:auto;-webkit-overflow-scrolling:touch;">';
            html += '<table class="monthly-table" style="min-width:900px;">';
            html += '<thead><tr>';
            html += `<th style="position:sticky;top:0;left:0;background:linear-gradient(135deg,#ea580c,#dc2626);z-index:30;border-right:3px solid #7c2d12;min-width:90px;font-size:10px;padding:8px 6px;font-weight:800;color:white;text-transform:uppercase;">📅 Datum</th>`;
            html += `<th style="position:sticky;top:0;background:linear-gradient(135deg,#ea580c,#dc2626);color:white;font-weight:800;border:1px solid #7c2d12;font-size:10px;padding:8px 6px;z-index:20;min-width:80px;text-transform:uppercase;">🏢 Odjel</th>`;
            sortNazivi.forEach((s, i) => {
                const isTotal = i === 19;
                const bg = isTotal ? 'linear-gradient(135deg,#7c2d12,#451a03)' : 'linear-gradient(135deg,#ea580c,#dc2626)';
                html += `<th style="position:sticky;top:0;min-width:55px;background:${bg};color:white;font-weight:700;border:1px solid #7c2d12;font-size:8.5px;padding:8px 3px;z-index:20;text-transform:uppercase;line-height:1.1;">${s}</th>`;
            });
            html += '</tr></thead><tbody>';
            ...
            const groupedByDate = {};
            data.data.forEach(row => {
                if (!groupedByDate[row.datum]) groupedByDate[row.datum] = [];
                groupedByDate[row.datum].push(row);
            });

            // Sortiraj datume descending (najnoviji prvi)
            const dates = Object.keys(groupedByDate).sort((a, b) => {
            ...
            // Grand totals za cijelu godinu
            const grandTotals = {};
            sortNazivi.forEach(s => grandTotals[s] = 0);

            dates.forEach(datum => {
                const rows = groupedByDate[datum];
                const dayName = getDayName(datum);
                const numCols = 2 + sortNazivi.length;

                // Zaglavlje datuma
                html += `<tr style="background:linear-gradient(135deg,#c2410c,#9a3412,#7c2d12);box-shadow:0 2px 8px rgba(124,45,18,0.4);">`;
                html += `<td colspan="${numCols}" style="font-weight:800;font-size:14px;padding:10px 12px;text-align:center;border-top:3px solid #451a03;color:white;letter-spacing:1px;text-shadow:0 2px 4px rgba(0,0,0,0.5);">📅 ${datum} - ${dayName}</td>`;
                html += '</tr>';

                // Dnevni totali
                const dailyTotals = {};
                sortNazivi.forEach(s => dailyTotals[s] = 0);

                // Data rows
                rows.forEach((row, idx) => {
                    const rowBg = idx % 2 === 0 ? '#fff7ed' : '#ffffff';
                    html += `<tr style="background:${rowBg};transition:all 0.15s ease;" onmouseover="this.style.background='#ffedd5'" onmouseout="this.style.background='${rowBg}'">`;
                    html += `<td style="font-weight:700;font-size:10px;position:sticky;left:0;background:${rowBg};z-index:10;border-right:2px solid #ea580c;padding:6px 5px;border:1px solid #fed7aa;color:#7c2d12;">${row.datum}</td>`;
                    html += `<td style="font-weight:600;font-size:10px;border:1px solid #fed7aa;padding:6px 5px;color:#9a3412;">${row.odjel}</td>`;

                    sortNazivi.forEach((s, j) => {
                        const val = row.sortimenti[s] || 0;
                        dailyTotals[s] += val;
                        grandTotals[s] += val;
                        const disp = val > 0 ? val.toFixed(2) : '-';
                        const fontWeight = val > 0 ? 'font-weight:600;color:#7c2d12;' : 'color:#d1d5db;';
                        html += `<td style="${fontWeight}border:1px solid #fed7aa;font-family:'Courier New',monospace;font-size:10px;text-align:right;padding:6px 3px;">${disp}</td>`;
                    });
                    html += '</tr>';
                });

                // Dnevni total row
                html += `<tr style="background:linear-gradient(to bottom,#fed7aa,#fdba74);box-shadow:0 1px 4px rgba(251,146,60,0.3);">`;
                html += `<td style="position:sticky;left:0;background:linear-gradient(to right,#fed7aa,#fdba74);z-index:10;border-right:2px solid #ea580c;padding:8px 5px;font-size:11px;font-weight:800;color:#7c2d12;">📊 UKUPNO</td>`;
                html += `<td style="background:transparent;border:1px solid #fb923c;"></td>`;
                sortNazivi.forEach(s => {
                    const val = dailyTotals[s];
                    const disp = val > 0 ? val.toFixed(2) : '-';
                    html += `<td style="border:1px solid #fb923c;font-family:'Courier New',monospace;font-size:10px;text-align:right;padding:7px 3px;font-weight:800;background:#fed7aa;color:#7c2d12;">${disp}</td>`;
                });
                html += '</tr>';
            });

            // Grand total za cijelu godinu
            html += `<tr style="background:linear-gradient(135deg,#16a34a,#059669,#047857);color:white;font-weight:800;border-top:4px solid #065f46;box-shadow:0 -2px 8px rgba(22,163,74,0.3);">`;
            html += `<td colspan="2" style="padding:12px;font-size:13px;text-align:center;font-weight:900;letter-spacing:1.5px;text-shadow:0 1px 3px rgba(0,0,0,0.4);">📈 UKUPNO ${year}</td>`;
            sortNazivi.forEach(s => {
                const val = grandTotals[s];
                const disp = val > 0 ? val.toFixed(2) : '-';
                html += `<td style="border:2px solid #7c2d12;font-family:'Courier New',monospace;text-align:right;padding:9px 3px;font-weight:800;font-size:11px;background:#dcfce7;color:#065f46;">${disp}</td>`;
            });
            html += '</tr>';
