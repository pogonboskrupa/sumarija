        ...
        async function checkManifest(skipInvalidation) {
            ...
            try {
                ...
                // Crvena tačka na ☁️ dugmetu — ima novih podataka koje korisnik
                // još nije preuzeo (gasi se u checkForNewData nakon preuzimanja)
                const seenVer = localStorage.getItem('manifest_seen_version');
                if (manifest.version && seenVer && String(manifest.version) !== seenVer) {
                    const dot = document.getElementById('check-data-dot');
        ...
        async function checkForNewData(event) {
            ...
            const icon = document.getElementById('check-data-icon');
            const dot  = document.getElementById('check-data-dot');

            if (!navigator.onLine) {
                if (typeof showWarning === 'function') {
                    showWarning('Offline', 'Nema mreže — koristim zadnje sačuvane podatke.');
                }
                return;
            }
        ...
        // drainOtpremaQueue) — poziva se sa OBA puta kojim app sazna da je
        // veza (opet) dobra: pravi browser 'online' event ISPOD, i periodični
        // _probeVeza() oporavak od slabe veze (setInterval ispod _probeVeza).
        // Ranije je drain bio ožičen SAMO na 'online' event — a taj event na
        // terenu često uopšte ne okine (telefon ostaje "online" po
        // navigator.onLine cijelo vrijeme, samo signal ne nosi podatke; baš
        // taj slučaj _probeVeza i postoji da uhvati). Bez ovoga bi unos
        // ostao zaglavljen u redu čekanja i kad signal stvarno proradi, sve
        // dok korisnik ne otvori "Moje sječe"/otpreme ili pošalje novi unos.
        function _drainQueuesIfLoggedIn() {
            if (!currentUser) return;
            setTimeout(() => {
                if (typeof drainSjecaQueue === 'function') drainSjecaQueue();
                if (typeof drainOtpremaQueue === 'function') drainOtpremaQueue();
                if (typeof drainEditSjecaQueue === 'function') drainEditSjecaQueue();
                if (typeof drainEditOtpremaQueue === 'function') drainEditOtpremaQueue();
            }, 2000); // odgoda — i 'online' event i probe znaju okinuti prije nego veza stvarno proradi
        }
        ...
        window.addEventListener('online', () => {
            ...
            // Nezavisno od toga koji je tab otvoren — za razliku od
            // osvježavanja taba ispod, ovo ne dira formu na ekranu.
            _drainQueuesIfLoggedIn();
        ...
        setInterval(() => {
            ...
            _probeVeza().then(ok => {
                ...
                // Isti red čekanja kao 'online' handler iznad — ovo je BAŠ
                // scenario radi kojeg _drainQueuesIfLoggedIn i postoji ovdje,
                // ne samo na 'online' event (vidi komentar uz tu funkciju).
                _drainQueuesIfLoggedIn();
        ...
        // Prikaži "Učitavam podatke" SAMO ako aktivni tab nema ništa renderovano —
        // inače korisnik gleda postojeće (keširane) podatke dok se tiho osvježava.
        // Ovim tab-switch NIKAD ne pokazuje loading ekran kad bilo kakav sadržaj postoji.
        function requestLoadingScreen(contentId) {
            const id = contentId || (window.TAB_CONTENT_MAP || {})[window.currentTab];
            const el = id ? document.getElementById(id) : null;
            if (el && el.children.length > 0) return; // ima sadržaja → bez loading ekrana
        ...
        // globalFullscreenEmergencyExit() je UKLONJEN zajedno sa svojim "✕"
        // dugmetom. Postojao je jer su vlastita "✕ Zatvori" dugmad fullscreen
        // tabova znala odskrolati van dohvata — ali je u praksi bio DRUGO dugme
        // za istu radnju na ekranu (kod Kubikatora tačno preko njegovog
        // "Zatvori"). Umjesto dupliranja, svaki fullscreen mod sad ima vlastiti
        // close pribijen za viewport (position:fixed) pa ne može odskrolati —
        // vidi objašnjenje uz .kub-header-row/.sih-close-btn u css/main.css.
        ...
        async function _preloadAllViewsImpl(silent = false, forceRefresh = false) {
            ...
            try {
                ...
                if (totalViews - totalFailed > 0) {
                    try {
                        localStorage.setItem('sumarija_last_full_refresh', String(Date.now()));
                        // Sinhronizuj "viđenu" manifest verziju — upravo smo preuzeli sve,
                        // pa ☁️ provjera ne treba javljati "novi podaci" za isto stanje
                        const m = JSON.parse(localStorage.getItem('manifest') || 'null');
                        if (m && m.version != null) localStorage.setItem('manifest_seen_version', String(m.version));
                        const dot = document.getElementById('check-data-dot');
                        if (dot) dot.style.display = 'none';
                    } catch(_) {}
                }
        ...
        let odjeliList = [];
        let preklasiranjaPodaci = []; // [{rowIndex, odjel, iz, u, kolicina, napomena, korisnik, datum}]
          ...
          try {
            ...
            if (parsedUser && savedPass) {
                ...
                const initialLoad = _probeVeza().then(() => {
                    ...
                    // Ako je app zatvorena/ubijena sa stavkama u redu čekanja
                    // (sječa/otprema uneseni offline, nikad poslani), do sada
                    // se ništa nije pokušalo poslati dok korisnik ručno ne
                    // otvori "Moje sječe/otpreme" ili ne pošalje novi unos —
                    // ako je već online pri sljedećem pokretanju, pokušaj
                    // odmah umjesto da unos i dalje čeka bez razloga.
                    _drainQueuesIfLoggedIn();
        ...
        async function loadZadnjih5DanaTable() {
            ...
            try {
                ...
                z5RawPrimke = primkeData.primke || [];
                z5RawOtpreme = otpremeData.otpreme || [];

                // Izvuci subote sa unosom
                z5SuboteSaUnosom = new Set();
                const normDate = d => (d && typeof d === 'string') ? d.trim() : null;
                const parseDate = s => { if (!s) return null; const [d,m,y] = s.split('.').map(Number); return new Date(y, m-1, d); };
                z5RawPrimke.forEach(p => { const d = normDate(p.datum); if (d && parseDate(d)?.getDay() === 6) z5SuboteSaUnosom.add(d); });
                z5RawOtpreme.forEach(o => { const d = normDate(o.datum); if (d && parseDate(d)?.getDay() === 6) z5SuboteSaUnosom.add(d); });

                // Renderiraj default prikaz (zadnjih 5 radnih dana)
                renderZadnjihPeriodTable(null, null);
        ...
        // Renderiraj tabelu za proizvoljni period (null = zadnjih 5 radnih dana)
        function renderZadnjihPeriodTable(customFrom, customTo) {
            const headerElem = document.getElementById('zadnjih5-dana-header');
            const bodyElem = document.getElementById('zadnjih5-dana-body');
            const periodElem = document.getElementById('zadnjih5-period');

            if (!z5RawPrimke) return; // Podaci još nisu učitani
            ...
            let validDatumi = new Set();
            let startDate, endDate;

            if (customFrom && customTo) {
                // Custom period — uključi sve datume od-do
                startDate = new Date(customFrom); startDate.setHours(0,0,0,0);
                endDate = new Date(customTo); endDate.setHours(0,0,0,0);
                const cur = new Date(startDate);
                while (cur <= endDate) { validDatumi.add(fmtDate(cur)); cur.setDate(cur.getDate()+1); }
            } else {
                // Default: zadnjih 5 radnih dana (pon-pet), ne uključuje danas
                const today = new Date(); today.setHours(0,0,0,0);
                const radniDani = [];
                const cur = new Date(today);
                cur.setDate(cur.getDate() - 1);
                while (radniDani.length < 5) {
                    if (cur.getDay() >= 1 && cur.getDay() <= 5) radniDani.push(new Date(cur));
                    cur.setDate(cur.getDate() - 1);
                }
                startDate = radniDani[radniDani.length - 1];
                endDate = radniDani[0];
                radniDani.forEach(d => validDatumi.add(fmtDate(d)));
                // Uključi subote sa unosom unutar perioda
                z5SuboteSaUnosom.forEach(s => {
                    const d = parseDate(s);
                    if (d && d >= startDate && d <= endDate) validDatumi.add(s);
                });
            }

            // Prikaži period u naslovu
            if (periodElem) {
                periodElem.textContent = `(${fmtShort(startDate)}–${fmtDate(endDate)})`;
            }
            ...
            const sjecaSortimenti = {}, otpremaSortimenti = {};
            sortimentiPrikaz.forEach(s => { sjecaSortimenti[s.display] = 0; otpremaSortimenti[s.display] = 0; });

            z5RawPrimke.forEach(primka => {
                const d = normDate(primka.datum);
                if (d && validDatumi.has(d)) {
                    sortimentiPrikaz.forEach(sp => { if (sp.keys.includes(primka.sortiment)) sjecaSortimenti[sp.display] += parseFloat(primka.kolicina) || 0; });
                }
            });
            z5RawOtpreme.forEach(otprema => {
                const d = normDate(otprema.datum);
                if (d && validDatumi.has(d)) {
                    sortimentiPrikaz.forEach(sp => { if (sp.keys.includes(otprema.sortiment)) otpremaSortimenti[sp.display] += parseFloat(otprema.kolicina) || 0; });
                }
            });

            let sjecaUkupno = 0, otpremaUkupno = 0;
            sortimentiPrikaz.forEach(sp => {
                if (!sp.display.startsWith('Σ')) { sjecaUkupno += sjecaSortimenti[sp.display]; otpremaUkupno += otpremaSortimenti[sp.display]; }
            });

            const razlikaSortimenti = {};
            sortimentiPrikaz.forEach(sp => { razlikaSortimenti[sp.display] = sjecaSortimenti[sp.display] - otpremaSortimenti[sp.display]; });
            const razlikaUkupno = sjecaUkupno - otpremaUkupno;

            // Header
            let headerHtml = '<tr style="background: linear-gradient(135deg, #047857 0%, #065f46 100%);">';
            headerHtml += '<th style="color: white; font-weight: 600; text-align: left; padding: 8px 10px; font-size: 13px;">Vrsta</th>';
            sortimentiPrikaz.forEach(sort => { headerHtml += `<th style="color: white; font-weight: 600; text-align: right; padding: 8px 6px; font-size: 12px;">${sort.display}</th>`; });
            headerHtml += '<th style="color: white; font-weight: 700; text-align: right; padding: 8px 10px; font-size: 13px; background: #064e3b;">UKUPNO</th></tr>';
            headerElem.innerHTML = headerHtml;

            // Body
            let bodyHtml = '';
            // Sječa
            bodyHtml += '<tr style="background: #f0fdf4;"><td style="font-weight: 600; color: #047857; padding: 8px 10px; font-size: 13px;">🌲 Sječa</td>';
            sortimentiPrikaz.forEach(sort => {
                const val = sjecaSortimenti[sort.display];
                bodyHtml += `<td style="text-align: right; padding: 8px 6px; font-family: 'Roboto Mono', monospace; font-size: 12px; color: ${val > 0 ? '#059669' : '#9ca3af'};">${val > 0 ? val.toFixed(2) : '-'}</td>`;
            });
            bodyHtml += `<td style="text-align: right; padding: 8px 10px; font-family: 'Roboto Mono', monospace; font-size: 13px; font-weight: 700; color: #047857; background: #d1fae5;">${sjecaUkupno > 0 ? sjecaUkupno.toFixed(2) : '-'}</td></tr>`;
            // Otprema
            bodyHtml += '<tr style="background: #fef2f2;"><td style="font-weight: 600; color: #dc2626; padding: 8px 10px; font-size: 13px;">🚚 Otprema</td>';
            sortimentiPrikaz.forEach(sort => {
                const val = otpremaSortimenti[sort.display];
                bodyHtml += `<td style="text-align: right; padding: 8px 6px; font-family: 'Roboto Mono', monospace; font-size: 12px; color: ${val > 0 ? '#dc2626' : '#9ca3af'};">${val > 0 ? val.toFixed(2) : '-'}</td>`;
            });
            bodyHtml += `<td style="text-align: right; padding: 8px 10px; font-family: 'Roboto Mono', monospace; font-size: 13px; font-weight: 700; color: #dc2626; background: #fecaca;">${otpremaUkupno > 0 ? otpremaUkupno.toFixed(2) : '-'}</td></tr>`;
            // Razlika
            bodyHtml += '<tr style="background: #f3f4f6;"><td style="font-weight: 600; color: #4b5563; padding: 8px 10px; font-size: 13px;">📊 Razlika (SJE−OTP)</td>';
            sortimentiPrikaz.forEach(sort => {
                const val = razlikaSortimenti[sort.display];
                bodyHtml += `<td style="text-align: right; padding: 8px 6px; font-family: 'Roboto Mono', monospace; font-size: 12px; color: #4b5563;">${val !== 0 ? (val > 0 ? '+' : '') + val.toFixed(2) : '-'}</td>`;
            });
            const rDisp = razlikaUkupno !== 0 ? (razlikaUkupno > 0 ? '+' : '') + razlikaUkupno.toFixed(2) : '-';
            bodyHtml += `<td style="text-align: right; padding: 8px 10px; font-family: 'Roboto Mono', monospace; font-size: 13px; font-weight: 700; color: #4b5563; background: #e5e7eb;">${rDisp}</td></tr>`;

            bodyElem.innerHTML = bodyHtml;
        }

        // Primijeni custom period iz date inputa
        function applyZadnjihPeriod() {
            const od = document.getElementById('zadnjih-od').value;
            const do_ = document.getElementById('zadnjih-do').value;
            if (!od || !do_) { alert('Molimo izaberite oba datuma.'); return; }
            if (od > do_) { alert('Datum "Od" mora biti prije datuma "Do".'); return; }
            renderZadnjihPeriodTable(od, do_);
        }
        ...
        // Vraća {sortiment: true} za RAZLIKA_MJERENJA unose za dati odjel.
        // Ti sortimenti se prikazuju u kartici ali se isključuju iz ukupnih agregata.
        function getRazlikeExclusions(odjelNaziv) {
            const excl = {};
            preklasiranjaPodaci
                .filter(p => p.odjel === odjelNaziv && p.tip === 'RAZLIKA_MJERENJA')
                .forEach(p => { excl[p.iz] = true; });
            return excl;
        }
        ...
        // exclusions: {sortiment: true} — isključuje sortiment iz agregata (TRUPCI Č, ČETINARI, LIŠĆARI, UKUPNO)
        //   ali NE mijenja individualnu vrijednost u spreadu (za prikaz u kartici).
        function buildCorrectedZaliha(z, exclusions) {
            if (!z) return {};
            const excl = exclusions || {};
            const val  = k => z[k] || 0;
            const agg  = k => excl[k] ? 0 : val(k); // isključeni sortimenti ne ulaze u agregate
        ...
        function renderPoslovodjaSjecaTable(data) {
            ...
            let headerHtml = '<tr style="background:#1e3a5f;">' +
                '<th style="min-width:80px;background:#1e3a5f;color:white;font-size:13px;">Datum</th>' +
                '<th style="min-width:80px;background:#1e3a5f;color:white;font-size:13px;">Odjel</th>' +
                '<th style="min-width:100px;background:#1e3a5f;color:white;font-size:13px;">Primač</th>';
            SORT_KOLONE_HEADER.forEach((s, idx) => {
                headerHtml += `<th style="min-width:55px; font-size:13px; text-align:right; white-space:nowrap; padding:8px 6px; ${_sortKolonaHeaderStil(idx)}">${s}</th>`;
            });
            headerHtml += '<th style="min-width:70px; text-align:right; font-size:13px; white-space:nowrap; background:#1e3a5f; color:white; border-left:2px solid rgba(255,255,255,.6);">UKUPNO Č+L</th></tr>';
            headerElem.innerHTML = headerHtml;
            ...
            data.forEach(item => {
                ...
                bodyHtml += `<td style="font-weight:600; color:#059669; font-size:13px;">${item.odjel}</td>`;
                bodyHtml += `<td style="font-size:13px;">${item.primac}</td>`;
                SORT_KOLONE.forEach(s => {
                ...
                bodyHtml += `<td style="text-align:right; font-family:'Courier New',monospace; font-size:13px; font-weight:700; color:#059669;">${item.ukupno > 0 ? item.ukupno.toFixed(2) : '-'}</td>`;
                bodyHtml += '</tr>';
            });
            ...
            bodyHtml += `<td colspan="3" style="font-weight:700; text-align:right; font-size:13px;">UKUPNO SJEČA:</td>`;
            SORT_KOLONE.forEach(s => {
                const val = grandTotals[s];
                bodyHtml += `<td style="text-align:right; font-family:'Courier New',monospace; font-size:12px; font-weight:700;">${val > 0 ? val.toFixed(2) : '-'}</td>`;
            });
            bodyHtml += `<td style="text-align:right; font-family:'Courier New',monospace; font-size:13px; font-weight:700;">${grandTotals['ukupno'].toFixed(2)}</td>`;
            bodyHtml += '</tr>';
        ...
        function buildSubtotalRow(datum, totals, numCols, bgColor, textColor) {
            ...
            html += `<td colspan="3" style="font-weight:700; text-align:right; color:${textColor}; font-size:13px;">Ukupno ${datum}:</td>`;
            SORT_KOLONE.forEach(s => {
                const val = totals[s];
                html += `<td style="text-align:right; font-family:'Courier New',monospace; font-size:12px; color:${textColor}; font-weight:700;">${val > 0 ? val.toFixed(2) : '-'}</td>`;
            });
            html += `<td style="text-align:right; font-family:'Courier New',monospace; font-size:13px; color:${textColor}; font-weight:700;">${totals['ukupno'].toFixed(2)}</td>`;
            html += '</tr>';
            return html;
        }
        ...
        function renderPoslovodjaOtpremaTabTable(data) {
            ...
            let headerHtml = '<tr style="background:#1e3a5f;">' +
                '<th style="min-width:80px;background:#1e3a5f;color:white;font-size:13px;">Datum</th>' +
                '<th style="min-width:80px;background:#1e3a5f;color:white;font-size:13px;">Odjel</th>' +
                '<th style="min-width:100px;background:#1e3a5f;color:white;font-size:13px;">Otpremač</th>' +
                '<th style="min-width:100px;background:#1e3a5f;color:white;font-size:13px;">Kupac</th>';
            SORT_KOLONE_HEADER.forEach((s, idx) => {
                headerHtml += `<th style="min-width:55px; font-size:13px; text-align:right; white-space:nowrap; padding:8px 6px; ${_sortKolonaHeaderStil(idx)}">${s}</th>`;
            });
            headerHtml += '<th style="min-width:70px; text-align:right; font-size:13px; white-space:nowrap; background:#1e3a5f; color:white; border-left:2px solid rgba(255,255,255,.6);">UKUPNO Č+L</th></tr>';
        ...
        function renderPrimaci(data) {
                ...
                const headerHTML = `
                    <tr>
                        <th style="position: sticky; left: 0; background: linear-gradient(135deg, #a7f3d0, #6ee7b7); color: #065f46; z-index: 20; border-right: 3px solid #059669; min-width: 150px;">
                            👷 Primač
                        </th>
                        ${data.mjeseci.map((m, idx) => `
                            <th class="right" style="min-width: 80px; background: linear-gradient(135deg, #a7f3d0, #6ee7b7); color: #065f46; font-weight: 700; border: 1px solid #059669;">
                                ${m}
                            </th>
                        `).join('')}
                ...
                const prosjekHeaderHTML = `
                    <tr>
                        <th style="position: sticky; left: 0; background: linear-gradient(135deg, #fde68a, #fbbf24); color: #78350f; z-index: 20; border-right: 3px solid #d97706; min-width: 150px;">
                            👷 Primač
                        </th>
                        ${data.mjeseci.map((m, idx) => `
                            <th class="right" style="min-width: 80px; background: linear-gradient(135deg, #fde68a, #fbbf24); color: #78350f; font-weight: 700; border: 1px solid #d97706;">
                                ${m}
                            </th>
                        `).join('')}
                        <th class="right" style="min-width: 100px; background: linear-gradient(135deg, #a7f3d0, #6ee7b7); color: #065f46; font-weight: 900; border: 2px solid #059669;">
                            📊 PROSJEK
                        </th>
                    </tr>
                `;
                ...
                const prosjekBodyHTML = data.primaci.map((p, idx) => {
                    ...
                    return `
                        <tr style="background: ${rowBg}; transition: all 0.2s;" onmouseover="this.style.background='${hoverBg}'" onmouseout="this.style.background='${rowBg}'">
                            ...
                            ${cells}
                            <td class="right" style="font-weight: 700; background: linear-gradient(to right, #fef3c7, #fde68a); border: 2px solid #d97706; padding: 10px; font-size: 11px; color: #78350f;">
                                ${ukupniProsjek} m³/dan
                            </td>
                        </tr>
                    `;
                ...
                const prosjekTotalsRow = `
                    <tr style="background: linear-gradient(to bottom, #fef3c7, #fde68a); color: #78350f; font-weight: 700; border-top: 3px solid #fbbf24;">
                ...
                const daniTotalsRow = `
                    <tr style="background: linear-gradient(to bottom, #d1fae5, #a7f3d0); color: #065f46; font-weight: 700; border-top: 3px solid #34d399;">
                        ...
                        ${daniMonthTotals.map(total => {
                            return `
                            <td class="right" style="border: 1px solid #6ee7b7; padding: 10px; font-size: 12px; font-weight: 700; color: #000000; font-family: 'Roboto Mono', ui-monospace, monospace;">
                                ${total > 0 ? total : '-'}
                            </td>`;
                        }).join('')}
                        <td class="right" style="background: #a7f3d0; border: 2px solid #34d399; padding: 12px; font-size: 13px; font-weight: 900; color: #000000; font-family: 'Roboto Mono', ui-monospace, monospace;">
            ...
            try {
                ...
                if (!data.data || data.data.length === 0) {
                    document.getElementById('primaci-daily-header').innerHTML = `
                        <tr>
                            <th style="background: linear-gradient(135deg, #a7f3d0, #6ee7b7); color: #065f46; padding: 12px; font-weight: 700;">
                ...
                const headerHTML = `
                    <tr>
                        ...
                        ${data.sortimentiNazivi.map(s => `
                            <th style="position: sticky; top: 0; min-width: 58px; background: linear-gradient(135deg, #fde68a, #fbbf24); color: #78350f; font-weight: 600; border: 1px solid #d97706; font-size: 9.5px; padding: 10px 4px; z-index: 20; box-shadow: 0 2px 6px rgba(120,53,15,0.1); text-transform: uppercase; line-height: 1.2; letter-spacing: 0.3px;">
                                ${s}
                            </th>
                        `).join('')}
                    </tr>
            ...
            try {
                ...
                if (!data.data || data.data.length === 0) {
                    document.getElementById('otpremaci-daily-header').innerHTML = `
                        <tr>
                            <th style="background: linear-gradient(135deg, #fde68a, #fbbf24); color: #78350f; padding: 12px;">
                ...
                const headerHTML = `
                    <tr>
                        ...
                        ${data.sortimentiNazivi.map(s => `
                            <th style="position: sticky; top: 0; min-width: 52px; background: linear-gradient(135deg, #fde68a 0%, #fbbf24 100%); color: #78350f; font-weight: 700; border: 1px solid #92400e; font-size: 8.5px; padding: 8px 3px; z-index: 20; box-shadow: 0 2px 4px rgba(120,53,15,0.1); text-transform: uppercase; line-height: 1.1;">
                                ${s}
                            </th>
                        `).join('')}
                    </tr>
                ...
                dates.forEach(datum => {
                    ...
                    rows.forEach((row, idx) => {
                        const rowBg = idx % 2 === 0 ? '#fffbeb' : '#ffffff';
                        const hoverBg = '#fef3c7';

                        const sortimentiCells = data.sortimentiNazivi.map(sortiment => {
                            const val = row.sortimenti[sortiment] || 0;
                            const displayVal = val > 0 ? val.toFixed(2) : '-';
                            const fontWeight = val > 0 ? 'font-weight: 600; color: #78350f;' : 'color: #d1d5db;';
                            return `<td style="${fontWeight} border: 1px solid #fde68a; font-family: 'Courier New', monospace; font-size: 10px; text-align: right; padding: 6px 3px; transition: all 0.15s;">${displayVal}</td>`;
                        }).join('');
                        ...
                        bodyHTML += `
                            <tr style="background: ${rowBg}; transition: all 0.15s ease;" onmouseover="this.style.background='${hoverBg}'; this.style.transform='scale(1.005)'; this.style.boxShadow='0 2px 8px rgba(217,119,6,0.15)';" onmouseout="this.style.background='${rowBg}'; this.style.transform='scale(1)'; this.style.boxShadow='none';">
                                ...
                                <td style="border: 1px solid #fde68a; color: #92400e; font-weight: 600; font-size: 10px; padding: 6px 5px; white-space: nowrap;">${escapeHtml(row.kupac) || '-'}</td>
                                ${sortimentiCells}
                            </tr>
                        `;
                    });
                    ...
                    const dailyTotalsCells = data.sortimentiNazivi.map(s => {
                        const val = dailyTotals[s];
                        const displayVal = val > 0 ? val.toFixed(2) : '-';
                        return `<td style="border: 1px solid #fbbf24; font-family: 'Courier New', monospace; text-align: right; padding: 7px 3px; font-size: 10px; font-weight: 800; color: #78350f; background: #fde68a;">${displayVal}</td>`;
                    }).join('');
                ...
                const grandTotalsCells = data.sortimentiNazivi.map(s => {
                    const val = grandTotals[s];
                    const displayVal = val > 0 ? val.toFixed(2) : '-';
                    return `<td style="border: 2px solid #92400e; font-family: 'Courier New', monospace; text-align: right; padding: 9px 3px; font-weight: 800; font-size: 11px; background: #fde68a; color: #78350f;">${displayVal}</td>`;
                }).join('');
        ...
        async function loadPrimaciByRadiliste() {
            try {
                ...
                headerHTML += `
                    <th style="background: linear-gradient(135deg, #047857, #065f46); color: white; padding: 12px; font-weight: 900; position: sticky; top: 0; z-index: 20;">
                        UKUPNO
                    </th>
                </tr>
                `;
        ...
        function _dinamikeSortimentTable(naslov, ikonBoja, bojaBg, tekstBoja, redovi, sortimentiNazivi) {
            ...
            redovi.forEach(red => {
                const cells = sortimentiNazivi.map(s => {
                    ...
                    const displayVal = val > 0 ? val.toFixed(2) : '-';
                    const isTotal = s === ukupnoKey;
                    return `<td style="${val > 0 ? 'font-weight:700; color:#1f2937;' : 'color:#d1d5db;'} ${isTotal ? 'background:#f9fafb; font-weight:800; border-left:2px solid #e2e8f0;' : ''}">${displayVal}</td>`;
                }).join('');
                bodyHTML += `<tr class="${red.istaknut ? 'dinamike-row-total' : ''}"><td>${escapeHtml(red.labela)}</td>${cells}</tr>`;
            });

            return `
                <div>
                    <p class="dinamike-subtable-title" style="color:${ikonBoja};">${naslov}</p>
                    <div class="dinamike-subtable-wrap">
                        <table class="dinamike-subtable"><thead>${headerHTML}</thead><tbody>${bodyHTML}</tbody></table>
                    </div>
                </div>
            `;
        }
                ...
                return `
                    <div class="izv-card">
                        <div class="izv-card-header">
                            <div class="izv-card-header-title">
                                <span class="izv-card-icon">🌲</span>
                                <div>
                                    <h4>Odjel ${escapeHtml(o.odjel)}</h4>
                                    <span class="izv-card-sub">
                        ...
                        <div class="izv-card-body">
                            <div class="dinamike-stat-strip">
                                ...
                                <div class="dinamike-stat-chip">🚛 Index otpreme: ${indexBadge(o.indexOtprema)}</div>
                            </div>
                            ${sjecaTable}
                            ${otpremaTable}
                        </div>
        ...
        // "Označi odjele kao pregledane" — klik na dugme u zaglavlju otvara
        // modal sa checkboxom po odjelu (svi trenutno prikazani, iz zadnjeg
        // učitanog _dinamikeIzvodjacaData). Potvrda šalje sve izabrane
        // odjednom i briše ih sa liste (isti set-dinamika-pregled poziv kao
        // ranije, samo grupno umjesto pojedinačno).
        function openDinamikePregledModal() {
            const odjeli = (_dinamikeIzvodjacaData && _dinamikeIzvodjacaData.odjeli) || [];
            const listEl = document.getElementById('dinamike-pregled-modal-list');
            const allCb = document.getElementById('dinamike-pregled-modal-select-all');
            const errEl = document.getElementById('dinamike-pregled-modal-error');
            if (!listEl) return;

            if (!odjeli.length) {
                listEl.innerHTML = '<div style="padding:16px;text-align:center;color:#9ca3af;font-size:13px;">Nema odjela za prikaz.</div>';
            } else {
                listEl.innerHTML = odjeli.map((o, idx) => `
                    <label style="display:flex; align-items:center; gap:10px; padding:9px 12px; border-bottom:1px solid #f1f5f9; cursor:pointer; font-size:13px;">
                        <input type="checkbox" class="dinamike-pregled-modal-cb" data-odjel="${escapeHtml(o.odjel)}" onchange="_dinamikePregledModalUpdateCount()">
                        <span>
                            <b>${escapeHtml(o.odjel)}</b>
                            <span style="color:#6b7280;"> — ${o.radiliste ? escapeHtml(String(o.radiliste)) + ' · ' : ''}${escapeHtml(o.izvodjac || '—')}</span>
                        </span>
                    </label>
                `).join('');
            }
            ...
            _dinamikePregledModalUpdateCount();

            document.getElementById('dinamike-pregled-modal').style.display = 'flex';
        }

        function closeDinamikePregledModal() {
            document.getElementById('dinamike-pregled-modal').style.display = 'none';
        }

        function _dinamikePregledModalToggleAll(checked) {
            document.querySelectorAll('.dinamike-pregled-modal-cb').forEach(cb => { cb.checked = checked; });
            _dinamikePregledModalUpdateCount();
        }

        function _dinamikePregledModalUpdateCount() {
            const boxes = Array.from(document.querySelectorAll('.dinamike-pregled-modal-cb'));
            const n = boxes.filter(cb => cb.checked).length;
            const countEl = document.getElementById('dinamike-pregled-modal-count');
            if (countEl) countEl.textContent = n ? `${n} izabrano` : '';
            const submitBtn = document.getElementById('dinamike-pregled-modal-submit');
            if (submitBtn) submitBtn.disabled = n === 0;
            const allCb = document.getElementById('dinamike-pregled-modal-select-all');
            if (allCb) allCb.checked = boxes.length > 0 && n === boxes.length;
        }

        async function _potvrdiDinamikePregled() {
            const boxes = Array.from(document.querySelectorAll('.dinamike-pregled-modal-cb')).filter(cb => cb.checked);
            const izabraniOdjeli = boxes.map(cb => cb.dataset.odjel).filter(Boolean);
            ...
            for (const odjel of izabraniOdjeli) {
                if (submitBtn) submitBtn.textContent = `⏳ Označavam (${done}/${izabraniOdjeli.length})...`;
                try {
            ...
            if (submitBtn) { submitBtn.textContent = 'Potvrdi'; submitBtn.disabled = false; }

            if (greske.length) {
                if (errEl) {
                    errEl.textContent = `Nije uspjelo za: ${greske.join(', ')}. Ostali su uspješno označeni.`;
                    errEl.style.display = 'block';
                }
            } else {
                closeDinamikePregledModal();
            }
        ...
        function loadPrimaciIzvodjacDetalj() {
            ...
            bodyHtml += '<tr class="ukupno-row" style="background:linear-gradient(135deg,#047857,#065f46);color:#fff;">' +
        ...
        let _primaciTrendState = { year: null, month: null, dailyRows: [], dailySort: [], loaded: false };
        let _primaciTrendWeekChart = null;
        // Boje po "leaf" sortimentu za slaganu (stacked) traku po sedmicama —
        // isti 8 sortimenata kao TREND_ZALIHA_LEAF_SORTIMENTI ispod (bez
        // duplog brojanja), četinari u zelenim, lišćari u braon/narandžastim
        // tonovima da se porodice sortimenata razlikuju na prvi pogled.
        const TREND_SORTIMENT_BOJE = {
            'TRUPCI Č':    '#047857',
            'CEL.DUGA':    '#10b981',
            'CEL.CIJEPANA':'#34d399',
            'ŠKART':       '#94a3b8',
            ...
            try {
                ...
                zalihaEl.innerHTML = !visokeZalihe.length
                    ...
                    : visokeZalihe.map((v, i) => (
                        '<div style="display:flex;align-items:center;gap:10px;padding:7px 2px;' + (i ? 'border-top:1px solid #f1f5f9;' : '') + '">' +
                        '<span style="flex:1;font-size:13px;font-weight:600;color:#374151;">' + v.odjel + ' <span style="color:#2563eb;font-weight:700;">— ' + v.sortiment + '</span></span>' +
        ...
        // Zbir po sedmicama za SVAKI "leaf" sortiment (TREND_ZALIHA_LEAF_SORTIMENTI —
        // isti 8 kao u "Brzi pregled" ispod, bez duplog brojanja klasa/agregata) u
        // trenutno izabranom mjesecu — grupisanje preko groupDataByWeeks (dijeli ga
        // i sedmični izvještaj sječe/otpreme, vidi loadSedmicniIzvjestajSjeca).
        // Vraća red po sedmici sa vrijednošću za svaki sortiment, za slaganu
        // (stacked) traku po sortimentu umjesto jedne linije za ručno izabran filter.
        function _primaciTrendWeekDataBySortiment() {
            ...
            return weeks.map(w => {
                const row = { label: 'Sed. ' + w.weekNumber + ' (' + w.weekStart.slice(0, 5) + '–' + w.weekEnd.slice(0, 5) + ')' };
                TREND_ZALIHA_LEAF_SORTIMENTI.forEach(s => {
                    let total = 0;
                    Object.keys(w.odjeliMap).forEach(odjel => { total += w.odjeliMap[odjel][s] || 0; });
                    row[s] = +total.toFixed(2);
                });
                return row;
            });
        }
        ...
        function renderPrimaciTrendCharts() {
            ...
            if (weekTitle) weekTitle.textContent = mjeseciNazPuni[_primaciTrendState.month] || '';

            const weekData = _primaciTrendWeekDataBySortiment();
            ...
            // "Prazno" znači nema sedmica s ijednom nenultom vrijednošću, ne samo prazan niz.
            const svePrazno = weekData.length === 0 || weekData.every(w => TREND_ZALIHA_LEAF_SORTIMENTI.every(s => !w[s]));
            if (svePrazno) {
            ...
            window.loadChartJs().then(() => {
                ...
                _primaciTrendWeekChart = new Chart(wCanvas.getContext('2d'), {
                    ...
                    data: {
                        ...
                        datasets: TREND_ZALIHA_LEAF_SORTIMENTI.map(s => ({
                            label: s,
                            data: weekData.map(w => w[s]),
                            backgroundColor: TREND_SORTIMENT_BOJE[s],
                            borderRadius: 4
                        }))
                    },
                    options: {
        ...
        function renderPrimaciSortimentiByPrimac(data, month, year) {
            ...
            let html = `<p style="color:#6b7280;font-size:13px;margin-bottom:16px;">📅 Prikazani podaci za: <strong style="color:#065f46;">${MJESECI_NAZIVI[month]} ${year}</strong></p>`;

            data.radilista.forEach(radiliste => {
                html += `<h4 style="background:linear-gradient(135deg, #047857, #065f46);color:#ecfdf5;padding:11px 18px;border-radius:8px;margin:24px 0 10px;font-weight:600;font-size:14px;letter-spacing:0.02em;">🏗️ ${escapeHtml(radiliste.naziv)}</h4>`;
                html += '<div style="overflow-x:auto;-webkit-overflow-scrolling:touch;border-radius:10px;box-shadow:0 1px 4px rgba(6,95,70,0.08);border:1px solid #d1fae5;">';
                ...
                sortNazivi.forEach((s, i) => {
                    const isTotal = i === 19;
                    const bg = isTotal ? 'linear-gradient(135deg, #047857, #065f46)' : 'linear-gradient(135deg, #fef3c7, #fde68a)';
                    const fg = isTotal ? '#ecfdf5' : '#92400e';
                ...
                radiliste.primaci.forEach((primac, idx) => {
                    ...
                    html += `<tr style="background:${bg};transition:background 0.15s;" onmouseover="this.style.background='#dcfce7'" onmouseout="this.style.background='${bg}'">`;
                    html += `<td style="font-weight:600;font-size:12px;border-bottom:1px solid #d1fae5;padding:9px 12px;color:#065f46;position:sticky;left:0;background:${bg};z-index:5;">${escapeHtml(primac.naziv)}</td>`;
                    primac.sortimentiVrijednosti.forEach((val, j) => {
                        ...
                        html += `<td style="${cellStyle}border-bottom:1px solid #d1fae5;font-family:'Courier New',monospace;font-size:11px;text-align:right;padding:7px 6px;">${disp}</td>`;
                    });
                    html += '</tr>';
                });

                // UKUPNO red za radilište
                html += `<tr style="background:#047857;">`;
                html += `<td style="font-weight:800;color:#ecfdf5;padding:10px 12px;border-top:2px solid #065f46;position:sticky;left:0;background:#047857;z-index:5;font-size:12px;">UKUPNO</td>`;
                radilisteUkupno.forEach((val, j) => {
                    const disp = val > 0 ? val.toFixed(2) : '-';
                    const isTotal = j === 19;
                    const cellStyle = isTotal ? 'background:#065f46;font-size:13px;font-weight:900;' : 'font-weight:700;';
                    html += `<td style="${cellStyle}color:#ecfdf5;border-top:2px solid #065f46;font-family:'Courier New',monospace;font-size:11px;text-align:right;padding:8px 6px;">${disp}</td>`;
                });
                html += '</tr>';
        ...
        function renderKupciSortableHeader(tabType, sortimentiNazivi, headerBgColor, headerBgColorAlt) {
            ...
            sortimentiNazivi.forEach(sortiment => {
                ...
                const sepStyle = ['F/L Č', 'F/L L', 'UKUPNO Č+L'].includes(sortiment) ? ' border-left: 3px solid rgba(255,255,255,0.6);' : '';
        ...
        function renderKupciGodisnjiTableBody() {
            ...
            sortedData.forEach((kupac, index) => {
                ...
                sortimentiNazivi.forEach(sortiment => {
                    ...
                    const sepStyle = ['F/L Č', 'F/L L', 'UKUPNO Č+L'].includes(sortiment) ? ' border-left: 3px solid #047857;' : '';
            ...
            sortimentiNazivi.forEach(sortiment => {
                ...
                const sepStyle = ['F/L Č', 'F/L L', 'UKUPNO Č+L'].includes(sortiment) ? ' border-left: 3px solid rgba(255,255,255,0.5);' : '';
        ...
        function renderKupciMjesecniTableBody() {
            ...
            sortedData.forEach((red, index) => {
                ...
                sortimentiNazivi.forEach(sortiment => {
                    ...
                    const sepStyle = ['F/L Č', 'F/L L', 'UKUPNO Č+L'].includes(sortiment) ? ' border-left: 3px solid #0369a1;' : '';
                    bodyHtml += `<td class="${medalClass}" style="text-align: right; font-family: 'Roboto Mono', ui-monospace, system-ui, monospace; font-weight: 500; color: ${color}; text-shadow: 0 0 1px rgba(255,255,255,0.8);${bgStyle}${sepStyle}">${display}</td>`;
                });
        ...
        // Keširani raw podaci za "zadnjih N dana" tabelu (dash)
        let z5RawPrimke = null;
        let z5RawOtpreme = null;
        let z5SuboteSaUnosom = new Set();
        ...
        function renderKupciKvartalniTableBody() {
            ...
            sortedData.forEach((kupac, index) => {
                ...
                sortimentiNazivi.forEach(sortiment => {
                    ...
                    const sepStyle = ['F/L Č', 'F/L L', 'UKUPNO Č+L'].includes(sortiment) ? ' border-left: 3px solid #7c3aed;' : '';
                    bodyHtml += `<td class="${medalClass}" style="text-align: right; font-family: 'Roboto Mono', ui-monospace, system-ui, monospace; font-weight: 500; color: ${color}; text-shadow: 0 0 1px rgba(255,255,255,0.8);${bgStyle}${sepStyle}">${display}</td>`;
                });
        ...
        // ============================================
        // 📈 KUPCI - STATISTIKA PO KUPCU (podtab)
        // ============================================
        // NAPOMENA (v1.4.20): prva verzija je za SVAKOG kupca dohvatala 12 mjeseci
        // sirovih otpremaci-daily podataka paralelno (do 12 GAS poziva) prije nego što
        // bi ISTA renderovala ijedan red — na sporijoj/slabijoj vezi to zna potrajati
        // predugo pa je prikaz djelovao "prazan" (korisnik ne sačeka da se Promise.all
        // razriješi). Sad se GLAVNI prikaz (mjesečno/kvartalno/po sortimentu/rang) puni
        // ODMAH iz već keširanih cache_kupci_<godina> agregata (isti podaci koje već
        // koriste Mjesečni/Kvartalni/Godišnji prikazi) — jedan fetch, gotovo instant.
        // Detaljniji dio (top odjeli/otpremači, period aktivnosti, prosječan razmak,
        // neaktivnost) i dalje treba sirove dnevne podatke pa se učitava NAKNADNO u
        // pozadini i dopunjava prikaz kad stigne, bez blokiranja glavnog prikaza.
        const _kupciStatCache = {};       // "godina|kupac" -> brzi stat (iz cache_kupci_<godina>)
        const _kupciStatDetailCache = {}; // "godina|kupac" -> detaljni stat (iz otpremaci-daily × 12)
        ...
        async function initKupciStatistikaControls() {
            ...
            const prevKupac = kupacSelect.value;

            // Lista kupaca za odabranu godinu — iz istog keša kao "Godišnji pregled"
            try {
                const url = buildApiUrl('kupci', { year });
                const data = await fetchWithCache(url, 'cache_kupci_' + year);
                const kupci = (data && data.godisnji) ? data.godisnji.map(k => k.kupac).filter(Boolean) : [];
                kupci.sort((a, b) => a.localeCompare(b, 'hr'));
        ...
        function _parseKupacDatum(datumStr, fallbackYear) {
            if (!datumStr) return null;
            const parts = String(datumStr).split('.');
            if (parts.length < 2) return null;
            const day = parseInt(parts[0]) || 1;
            const month = (parseInt(parts[1]) || 1) - 1;
            const yr = parts[2] ? parseInt(parts[2]) : fallbackYear;
            const d = new Date(yr, month, day);
            return isNaN(d.getTime()) ? null : d;
        }

        // BRZI stat — iz već keširanog cache_kupci_<godina> (isti podaci kao Mjesečni/
        // Godišnji prikaz). Jedan fetch, gotovo instant jer je taj keš skoro uvijek topao.
        function computeKupacStatistikaFast(kupacName, year, kupciData) {
            const kupacLower = kupacName.toLowerCase();
            const mjeseciNazivi = ["Januar","Februar","Mart","April","Maj","Juni","Juli","August","Septembar","Oktobar","Novembar","Decembar"];
            ...
            const sortimentiNaziviSve = (kupciData && kupciData.sortimentiNazivi) ? kupciData.sortimentiNazivi : [];

            const godRow = godisnji.find(k => (k.kupac || '').toLowerCase() === kupacLower) || null;
            const mjesecniRedovi = mjesecni.filter(m => (m.kupac || '').toLowerCase() === kupacLower);
            const mjesecniPoNazivu = new Map(mjesecniRedovi.map(m => [m.mjesec, m]));

            const monthly = mjeseciNazivi.map((naziv, month) => {
                const m = mjesecniPoNazivu.get(naziv);
                return {
                    month, naziv,
                    count: m ? (m.brOtpremnica || 0) : 0,
                    ukupno: m ? (m.ukupno || 0) : 0,
                    sortimenti: m ? (m.sortimenti || {}) : {},
                };
            });
            ...
            const sortimentTotals = godRow ? (godRow.sortimenti || {}) : {};
            const sortimentCounts = godRow ? (godRow.sortimentiCount || {}) : {};
            const totalCount = godRow ? (godRow.brOtpremnica || 0) : 0;
            const totalUkupno = godRow ? (godRow.ukupno || 0) : 0;
            const aktivnihMjeseci = monthly.filter(m => m.count > 0).length;

            // Rang — godisnji je već sortiran po ukupno opadajuće (backend), ali
            // sortiramo ponovo da bude sigurno tačno bez obzira na izvor podataka
            let rang = null;
            if (godRow) {
                const sorted = [...godisnji].sort((a, b) => (b.ukupno || 0) - (a.ukupno || 0));
                const idx = sorted.findIndex(k => (k.kupac || '').toLowerCase() === kupacLower);
                if (idx !== -1) rang = { rang: idx + 1, ukupnoKupaca: sorted.length };
            }

            return {
                monthly, quarterly, sortimentiNazivi: sortimentiNaziviSve, sortimentTotals, sortimentCounts,
                totalCount, totalUkupno, aktivnihMjeseci, rang,
            };
        }

        // DETALJNI stat — treba sirove dnevne podatke (top odjeli/otpremači, tačan
        // period aktivnosti, prosječan razmak) pa se dohvata naknadno u pozadini.
        async function computeKupacDetalji(kupacName, year) {
            const kupacLower = kupacName.toLowerCase();

            const monthResults = await Promise.all(Array.from({ length: 12 }, async (_, month) => {
                const url = buildApiUrl('otpremaci-daily', { year, month });
                const cKey = `cache_otpremaci_daily_${year}_${month}`;
                try {
                    const data = await fetchWithCache(url, cKey);
                    if (data && Array.isArray(data.data)) {
                        return data.data.filter(r => r.kupac && r.kupac.toLowerCase() === kupacLower);
                    }
                } catch (e) { /* mjesec bez podataka/mreže — tretiraj kao prazan */ }
                return [];
            }));
            ...
            monthResults.forEach(rows => rows.forEach(row => {
                let rowUkupno = 0;
                Object.keys(row.sortimenti || {}).forEach(s => { rowUkupno += (row.sortimenti[s] || 0); });

                const odjelKey = row.odjel || '-';
                if (!odjeliMap.has(odjelKey)) odjeliMap.set(odjelKey, { ukupno: 0, count: 0 });
                const oA = odjeliMap.get(odjelKey); oA.ukupno += rowUkupno; oA.count++;

                const otpKey = row.otpremac || '-';
                if (!otpremaciMap.has(otpKey)) otpremaciMap.set(otpKey, { ukupno: 0, count: 0 });
                const tA = otpremaciMap.get(otpKey); tA.ukupno += rowUkupno; tA.count++;

                const d = _parseKupacDatum(row.datum, year);
                if (d) {
            ...
            const topOdjeli = Array.from(odjeliMap.entries())
                .map(([odjel, v]) => ({ odjel, ...v })).sort((a, b) => b.ukupno - a.ukupno).slice(0, 8);
            const topOtpremaci = Array.from(otpremaciMap.entries())
                .map(([otpremac, v]) => ({ otpremac, ...v })).sort((a, b) => b.ukupno - a.ukupno).slice(0, 8);

            return { topOdjeli, topOtpremaci, firstDate, lastDate, avgGapDays, distinctDaysCount: distinctDays.length };
        }
        ...
        function renderKupciStatistika(stat, kupacName, year) {
            ...
            let html = '<div id="kupac-stat-inactive-slot"></div>';
            ...
            html += `<div class="summary-card green">
                ...
                <div class="summary-card-subtitle">${stat.aktivnihMjeseci} od 12 mjeseci aktivno</div>
            </div>`;
            html += `<div class="summary-card blue">
                ...
                <div class="summary-card-subtitle">Prosjek ${prosjek.toFixed(2)} m³/otpremnica</div>
            </div>`;
            html += `<div class="summary-card">
                <div class="summary-card-title">Najbolji mjesec</div>
                <div class="summary-card-value" style="font-size:20px;">${najboljiMjesec && najboljiMjesec.ukupno > 0 ? najboljiMjesec.naziv : '—'}</div>
                <div class="summary-card-subtitle">${najboljiMjesec && najboljiMjesec.ukupno > 0 ? najboljiMjesec.ukupno.toFixed(2) + ' m³' : ''}</div>
            </div>`;
            html += `<div class="summary-card">
            ...
            html += `<div class="summary-card" id="kupac-stat-period-card">
                <div class="summary-card-title">Period aktivnosti</div>
                <div class="summary-card-value" style="font-size:15px;">⏳ učitavam...</div>
            </div>`;
            html += `<div class="summary-card" id="kupac-stat-gap-card">
            ...
            html += `<div class="enterprise-card" style="margin-top:16px;">
                <div class="enterprise-card-header"><div><h2>📈 Mjesečni trend</h2><span class="card-subtitle">Ukupna količina (m³) po mjesecu</span></div></div>
                <div class="enterprise-card-body"><div style="position:relative;height:300px;"><canvas id="kupac-stat-chart"></canvas></div></div>
            </div>`;
            ...
            // Po sortimentima — broj otpremnica i količina
            if (stat.sortimentiNazivi.length) {
                const sortedSort = stat.sortimentiNazivi
                    .filter(s => (stat.sortimentTotals[s] || 0) > 0)
                    .sort((a, b) => (stat.sortimentTotals[b] || 0) - (stat.sortimentTotals[a] || 0));
                html += `<div class="enterprise-card" style="margin-top:16px;">
                    <div class="enterprise-card-header"><div><h2>🪵 Po sortimentima</h2><span class="card-subtitle">Broj otpremnica i ukupna količina po sortimentu</span></div></div>
                    <div class="enterprise-card-body"><div style="overflow-x:auto;">
                    <table class="kupci-table" style="width:100%;">
                    <thead><tr><th>Sortiment</th><th class="right">Broj otpremnica</th><th class="right ukupno-col">Ukupno m³</th><th class="right">% od ukupno</th></tr></thead>
                    <tbody>`;
                sortedSort.forEach(s => {
                    const val = stat.sortimentTotals[s] || 0;
                    const cnt = stat.sortimentCounts[s] || 0;
                    const pct = stat.totalUkupno > 0 ? (val / stat.totalUkupno * 100) : 0;
                    html += `<tr><td>${s}</td><td class="right">${cnt}</td><td class="right ukupno-col">${val.toFixed(2)}</td><td class="right">${pct.toFixed(1)}%</td></tr>`;
                });
                html += `</tbody></table></div></div></div>`;
            }
            ...
            content.innerHTML = html;
            requestAnimationFrame(() => renderKupacStatChart(stat));
        }

        // Dopuni prikaz detaljima koji zahtijevaju sirove dnevne podatke (poziva se
        // naknadno, u pozadini, bez blokiranja glavnog prikaza)
        function renderKupacDetaljiInto(detalji, year) {
            const periodCard = document.getElementById('kupac-stat-period-card');
            const gapCard = document.getElementById('kupac-stat-gap-card');
            const slot = document.getElementById('kupac-stat-detalji-slot');
            const inactiveSlot = document.getElementById('kupac-stat-inactive-slot');
            if (!slot) return; // korisnik je već otišao s ovog prikaza

            if (periodCard) {
                periodCard.querySelector('.summary-card-value').innerHTML =
                    `<span style="font-size:15px;">${_fmtKupacDatum(detalji.firstDate)} – ${_fmtKupacDatum(detalji.lastDate)}</span>`;
            }
            if (gapCard) {
                gapCard.querySelector('.summary-card-value').innerHTML =
                    `<span style="font-size:15px;">${detalji.avgGapDays != null ? detalji.avgGapDays.toFixed(1) + ' d' : '—'}</span>`;
                const sub = document.createElement('div');
                sub.className = 'summary-card-subtitle';
                sub.textContent = detalji.distinctDaysCount > 1 ? `između ${detalji.distinctDaysCount} dana isporuke` : 'nedovoljno podataka';
                gapCard.appendChild(sub);
            }

            // Neaktivnost — ima smisla samo za tekuću godinu (poređenje sa "danas")
            if (inactiveSlot) {
                const nowYear = new Date().getFullYear();
                const daysSinceLast = detalji.lastDate ? Math.floor((Date.now() - detalji.lastDate.getTime()) / 86400000) : null;
                const isInactive = (year === nowYear) && daysSinceLast !== null && daysSinceLast > 60;
                if (isInactive) {
                    inactiveSlot.innerHTML = `<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:10px;padding:12px 16px;margin-bottom:16px;display:flex;align-items:center;gap:10px;">
                        <span style="font-size:20px;">⚠️</span>
                        <span style="color:#991b1b;font-size:13px;font-weight:600;">Kupac neaktivan ${daysSinceLast} dana — zadnja otprema ${_fmtKupacDatum(detalji.lastDate)}.</span>
                    </div>`;
                }
            }

            let html = `<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:16px;">`;
            html += `<div class="enterprise-card"><div class="enterprise-card-header"><div><h2>🏭 Top odjeli</h2></div></div>
                <div class="enterprise-card-body"><div style="overflow-x:auto;">
                <table class="kupci-table" style="width:100%;"><thead><tr><th>Odjel</th><th class="right">Otpremnica</th><th class="right ukupno-col">m³</th></tr></thead><tbody>`;
            if (detalji.topOdjeli.length) {
                detalji.topOdjeli.forEach(o => {
            ...
            html += `<div class="enterprise-card"><div class="enterprise-card-header"><div><h2>🚛 Top otpremači</h2></div></div>
                <div class="enterprise-card-body"><div style="overflow-x:auto;">
                <table class="kupci-table" style="width:100%;"><thead><tr><th>Otpremač</th><th class="right">Otpremnica</th><th class="right ukupno-col">m³</th></tr></thead><tbody>`;
            if (detalji.topOtpremaci.length) {
                detalji.topOtpremaci.forEach(o => {
        ...
        function renderKupacStatChart(stat) {
            if (typeof window.loadChartJs !== 'function') return;
            window.loadChartJs().then(() => {
                const canvas = document.getElementById('kupac-stat-chart');
                if (!canvas) return;
                if (_kupacStatChart) { _kupacStatChart.destroy(); _kupacStatChart = null; }
                const existing = Chart.getChart(canvas);
                if (existing) existing.destroy();

                _kupacStatChart = new Chart(canvas.getContext('2d'), {
                    type: 'bar',
                    data: {
        ...
        // ========================================
        // PREKLASIRANJE - Admin UI
        // ========================================

        const SORTIMENTI_INDIVIDUALNI = [
            "F/L Č","I Č","II Č","III Č","RD",
        ...
        function initPreklasiranjeAdminSection() {
            ...
            if (isAdmin) renderPreklasiranjaTabelaAdmin();
        }
        ...
        function renderPreklasiranjaTabelaAdmin() {
            ...
            preklasiranjaPodaci.forEach(p => {
                ...
                const tipBadge = isRazlika
                    ? `<span style="background:#fef9c3;color:#92400e;border-radius:4px;padding:2px 6px;font-size:11px;white-space:nowrap;">Razlika mj.</span>`
                ...
                const sortimentCell = isRazlika
                    ? `<td style="padding:7px 10px;border:1px solid #e5e7eb;font-style:italic;color:#92400e;background:#fef9c3;">${p.iz}</td>`
        ...
        function onPreklTipChange() {
            ...
            const kolicContainer = document.getElementById('prekl-kolicina-container');
            ...
            if (kolicContainer) kolicContainer.style.display = isRazlika ? 'none' : '';
            izLabel.textContent = (isRazlika ? 'Sortiment *' : 'Iz sortimenta *');
            tipPreklLabel.style.borderColor = isRazlika ? '#d1d5db' : '#1e3a5f';
            tipRazlikaLabel.style.borderColor = isRazlika ? '#d97706' : '#d1d5db';
        }
        ...
        function openPreklasiranjeModal() {
            ...
            const odjeli = stanjeZalihaData.map(o => o.odjel).filter(Boolean).sort();
            odjeli.forEach(o => {
                odjelSel.innerHTML += `<option value="${o}">${o}</option>`;
            });

            // Populate sortimenti dropdowns
            const izSel = document.getElementById('prekl-iz');
            const uSel = document.getElementById('prekl-u');
            izSel.innerHTML = '<option value="">— Odaberi —</option>';
            uSel.innerHTML = '<option value="">— Odaberi —</option>';
            SORTIMENTI_INDIVIDUALNI.forEach(s => {
                izSel.innerHTML += `<option value="${s}">${s}</option>`;
                uSel.innerHTML += `<option value="${s}">${s}</option>`;
            });

            document.getElementById('prekl-kolicina').value = '';
            document.getElementById('prekl-napomena').value = '';
            document.getElementById('prekl-error').style.display = 'none';
            document.getElementById('prekl-submit-btn').disabled = false;
            document.getElementById('preklasiranje-modal').style.display = 'flex';
        }

        function filterPreklUSort() {
            const izVal = document.getElementById('prekl-iz').value;
            const uSel = document.getElementById('prekl-u');
            const current = uSel.value;
            uSel.innerHTML = '<option value="">— Odaberi —</option>';
            SORTIMENTI_INDIVIDUALNI.forEach(s => {
                if (s !== izVal) uSel.innerHTML += `<option value="${s}">${s}</option>`;
            });
            if (current && current !== izVal) uSel.value = current;
        }

        function closePreklasiranjeModal() {
            document.getElementById('preklasiranje-modal').style.display = 'none';
        }
        ...
        async function submitPreklasiranje() {
            ...
            if (tip === 'PREKLASIRANJE') {
                if (!kolicina) {
                    errEl.textContent = 'Unesite količinu.';
                    errEl.style.display = 'block';
                    return;
                }
                if (!u) {
            ...
            const btn = document.getElementById('prekl-submit-btn');
            btn.disabled = true;
        ...
        // Obriši SVE pending unose (dugme "Obriši sve" u Dodani unosi) — BUGFIX:
        // dugme je pozivalo ovu funkciju koja nikad nije postojala (ReferenceError,
        // klik nije radio ništa), iako backend endpoint delete-all-pending postoji.
        function deleteAllPendingUnosi() {
            showConfirmModal(
                'Potvrda brisanja SVIH unosa',
                'Da li ste SIGURNI da želite obrisati SVE dodane unose (sječa i otprema)? Ova akcija se ne može poništiti!',
                async function() {
                    try {
                        const formData = new URLSearchParams();
                        ...
                        if (result.success) {
                            showSuccess('Uspjeh', result.message || 'Svi unosi obrisani.');
                            loadPendingUnosi();
                        } else {
                            throw new Error(result.error || 'Unknown error');
                        }
        ...
        // ========== OFFLINE RED ČEKANJA — IZMJENA već poslane sječe/otpreme
        // (submitEditSjeca/submitEditOtprema) ==========
        // Isti obrazac kao red čekanja za NOVE unose iznad — ranije je
        // submitEditSjeca/submitEditOtprema slao "goli" fetch bez ikakvog
        // reda čekanja, pa je izmjena na terenu bez signala jednostavno
        // nestajala (korisnik bi vidio grešku i morao je ručno ponoviti,
        // ako se uopšte sjeti). rowIndex (ne uuid) je ovdje prirodni
        // dedup ključ — dvije izmjene ISTOG reda dok je prva još u redu se
        // upsertuju (druga zamjenjuje prvu), isto kao _queueUpsert iznad.
        function _editSjecaQueueKey()   { return 'queue_edit_sjeca_'   + ((currentUser && currentUser.username) || 'anon'); }
        function _editOtpremaQueueKey() { return 'queue_edit_otprema_' + ((currentUser && currentUser.username) || 'anon'); }

        async function _posaljiIzmjenuSjece(rowIndex, fields) {
            const formData = new URLSearchParams();
            Object.keys(fields).forEach(k => formData.append(k, fields[k]));
            let result;
            try {
                ...
                _queueUpsert(_editSjecaQueueKey(), String(rowIndex), fields, null);
                return { queued: true };
            }
            return result; // server je ODGOVORIO — uspjeh ili eksplicitno odbijanje
        }

        async function _posaljiIzmjenuOtpreme(rowIndex, fields) {
            const formData = new URLSearchParams();
            Object.keys(fields).forEach(k => formData.append(k, fields[k]));
            let result;
            try {
                ...
                _queueUpsert(_editOtpremaQueueKey(), String(rowIndex), fields, null);
                return { queued: true };
            }
            return result;
        }

        let _editSjecaDraining = false, _editOtpremaDraining = false;

        async function drainEditSjecaQueue() {
            if (_editSjecaDraining || !navigator.onLine) return;
            let q = _loadQueue(_editSjecaQueueKey());
            if (!q.length) return;
            _editSjecaDraining = true;
            let poslato = 0;
            try {
                while (q.length) {
                    const stavka = q[0];
                    const result = await _posaljiIzmjenuSjece(stavka.uuid, stavka.fields);
                    if (result.queued) break; // mreža i dalje ne radi, pokušaj kasnije
                    q.shift();
                    _saveQueue(_editSjecaQueueKey(), q);
                    if (result.success) {
                        poslato++;
                    } else if (typeof showError === 'function') {
                        showError('Izmjena sječe nije poslana', (result.error || 'Nepoznata greška') +
                            ' (odjel ' + (stavka.fields.odjel || '') + ', ' + (stavka.fields.datum || '') + ')');
                    }
                }
            } finally {
                _editSjecaDraining = false;
            }
            if (poslato) {
                if (typeof showSuccess === 'function') {
                    showSuccess('Izmjena poslana', poslato + (poslato === 1 ? ' sačuvana izmjena je' : ' sačuvane izmjene su') + ' stigla na server.');
                }
                clearCacheByPattern('primac'); clearCacheByPattern('primaci'); clearCacheByPattern('dashboard');
                clearCacheByPattern('izvjestaji'); clearCacheByPattern('sedmicni_sjeca'); clearCacheByPattern('stanje_odjela');
                clearCacheByPattern('stanje_zaliha'); clearCacheByPattern('my_sjece');
        ...
        async function drainEditOtpremaQueue() {
            if (_editOtpremaDraining || !navigator.onLine) return;
            let q = _loadQueue(_editOtpremaQueueKey());
            if (!q.length) return;
            _editOtpremaDraining = true;
            let poslato = 0;
            try {
                while (q.length) {
                    const stavka = q[0];
                    const result = await _posaljiIzmjenuOtpreme(stavka.uuid, stavka.fields);
                    if (result.queued) break;
                    q.shift();
                    _saveQueue(_editOtpremaQueueKey(), q);
                    if (result.success) {
                        poslato++;
                    } else if (typeof showError === 'function') {
                        showError('Izmjena otpreme nije poslana', (result.error || 'Nepoznata greška') +
                            ' (odjel ' + (stavka.fields.odjel || '') + ', ' + (stavka.fields.datum || '') + ')');
                    }
                }
            } finally {
                _editOtpremaDraining = false;
            }
            if (poslato) {
                if (typeof showSuccess === 'function') {
                    showSuccess('Izmjena poslana', poslato + (poslato === 1 ? ' sačuvana izmjena je' : ' sačuvane izmjene su') + ' stigla na server.');
        ...
        async function submitEditSjeca(event) {
            ...
            var rowIndex = document.getElementById('edit-sjeca-rowIndex').value;
            try {
                var fields = {
                    path: 'update-pending',
                    username: currentUser.username,
                    ...
                    tip: 'sjeca',
                    rowIndex: rowIndex,
                    datum: document.getElementById('edit-sjeca-datum').value,
                ...
                sortimentiKeys.forEach(function(key) {
                    var fieldId = 'edit-sjeca-' + key.replace(/\//g, '').replace(/ /g, '-');
                    fields[key] = document.getElementById(fieldId).value;
                });

                // _posaljiIzmjenuSjece na mrežnu grešku SAČUVA izmjenu u red
                // čekanja (isto kao Dodaj sječu) umjesto da je izgubi — vidi
                // komentar uz "OFFLINE RED ČEKANJA — IZMJENA" iznad.
                var result = await _posaljiIzmjenuSjece(rowIndex, fields);

                if (result.queued) {
                    messageDiv.innerHTML = '💾 Sačuvano lokalno — nema signala. Izmjena će se automatski poslati čim se veza vrati.';
                    messageDiv.style.background = '#fef3c7';
                    messageDiv.style.color = '#92400e';
                    messageDiv.classList.remove('hidden');
                    if (window._tabRenderTime) delete window._tabRenderTime['my-sjece'];
                    setTimeout(function() { switchTab('my-sjece'); }, 2000);
                    return;
                }
        ...
        async function submitEditOtprema(event) {
            ...
            var rowIndex = document.getElementById('edit-otprema-rowIndex').value;
            try {
                var fields = {
                    path: 'update-pending',
                    username: currentUser.username,
                    ...
                    tip: 'otprema',
                    rowIndex: rowIndex,
                    datum: document.getElementById('edit-otprema-datum').value,
                ...
                // _posaljiIzmjenuOtpreme na mrežnu grešku SAČUVA izmjenu u red
                // čekanja (isto kao Dodaj otpremu) umjesto da je izgubi — vidi
                // komentar uz "OFFLINE RED ČEKANJA — IZMJENA" iznad.
                var result = await _posaljiIzmjenuOtpreme(rowIndex, fields);
                ...
                if (result.queued) {
                    ...
                    setTimeout(function() { switchTab('my-otpreme'); }, 2000);
                    return;
                }
