...
## 📦 POTPUNA PRELOAD POKRIVENOST (dopuna 05.07.2026.)

Revizija: popisani SVI keš ključevi koje loaderi čitaju po ulozi i upoređeni s preload listama. Nađene i zatvorene rupe (razlog zašto se "neki tabovi offline ne prikazuju"):

**Ključno otkriće:** svi izvještaj podtabovi (sedmični, sedmični po radniku, mjesečni, sedmicni-sjeca/otprema) dijele ISTE URL-ove `primaci-daily`/`otpremaci-daily` — jedan fetch + `alsoCache` puni sve njihove ključeve bez ijednog dodatnog API poziva.

| Uloga | Dodano u preload |
|-------|------------------|
| Admin | izvještaji ×6 ključeva (alsoCache na Daily), `cache_otpreme_karta` (mapa!), `cache_preklasiranja`, GeoJSON fetch → SW keš (offline mapa bez posjete tabu) |
| Poslovođa | izvještaji ×6 ključeva, `cache_preklasiranja`, `cache_odjeli_list` |
| Operativa | `cache_odjeli_*` + `cache_stanje_zaliha` (bez njih Operativa tab offline prazan!), dashboard mN varijanta, primke/otpreme + dash varijante, izvještaji ×4 |
| Primac | `cache_odjeli_list` (dropdown forme Dodaj sječu) |
| Otpremac | `cache_odjeli_list` (dropdown forme Dodaj otpremu) |

**Poznato ograničenje O6:** preload kešira samo TEKUĆI mjesec/godinu za mjesečne prikaze — promjena mjeseca u selectu offline i dalje traži mrežu za nekeširane mjesece. Keširanje svih 12 mjeseci bi značilo 24+ dodatnih sporih GAS poziva; svjesno preskočeno.

**Poznato ograničenje O7:** "Pregled sječe" po pojedinom primaču (admin, `cache_primac_detail_admin_<ime>_<god>`) kešira se tek pri odabiru tog primača — preload svih primaca bi bio N dodatnih poziva.
